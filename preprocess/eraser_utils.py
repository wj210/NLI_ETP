...
"""
CODE FROM https://github.com/jayded/eraserbenchmark
"""
...
def load_documents(data_dir: str, docids: Set[str] = None,full_text = False,split_sentence=False,attack=False) -> Dict[str, List[List[str]]]:
    ...
    if os.path.exists(os.path.join(data_dir, 'docs.jsonl')):
        ...
        return load_documents_from_file(data_dir, docids,full_text=full_text)
    
    if attack:
        docs_dir = os.path.join(data_dir, 'attack_docs')
    else:
...
def load_documents_from_file(data_dir: str, docids: Set[str] = None,full_text = False) -> Dict[str, List[List[str]]]:
    ...
    for d in docids:
        ...
        if full_text:
            tokenized = " ".join([l.strip() for l in lines])
        else:
